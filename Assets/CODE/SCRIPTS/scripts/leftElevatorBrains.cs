...
public class leftElevatorBrains : UdonSharpBehaviour
{
    ...
    // so floors are 0(basement), 1(firstfloor), 2(secondfloor), 3(thirdFloor), 4(fourthFloor), 5(fifthFloor), 6(sixthFloor)
    ...
    {
        if (loadElevatorFloor == 0 && isGoing == false)
        {
            F0();
            isGoing = false;
            Debug.Log("sendCurrentFloor0L");
        }
        else if (loadElevatorFloor == 1)
        {
            F1();
            isGoing = false;
            Debug.Log("sendCurrentFloor1L");
        }
        else if (loadElevatorFloor == 2)
        {
            F2();
            isGoing = false;
            Debug.Log("sendCurrentFloor2L");
        }
        else if (loadElevatorFloor == 3)
        {
            F3();
            isGoing = false;
            Debug.Log("sendCurrentFloor3L");
        }
        else if (loadElevatorFloor == 4)
        {
            F4();
            isGoing = false;
            Debug.Log("sendCurrentFloor4L");
        }
        else if (loadElevatorFloor == 5)
        {
            F5();
            isGoing = false;
            Debug.Log("sendCurrentFloor5L");
        }
    }
