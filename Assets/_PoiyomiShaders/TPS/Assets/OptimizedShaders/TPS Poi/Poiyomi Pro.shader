Shader "Hidden/Locked/.poiyomi/Old Versions/8.2/Poiyomi Pro/8b93b8a55f9aa1249939c948ef1d0d0e"
{
	Properties
	{
		...
		[Space(4)]
		[sRGBWarning(true)]_ALDecalColorMask ("Color & Mask--{reference_properties:[_ALDecalColorMaskPan, _ALDecalColorMaskUV]}", 2D) = "white" { }
 ...
 #define OPTIMIZER_ENABLED 
			...
			#define ID_ENHANCED ((14 - 1) * ID_ENHANCED_STEPS + ID_ENHANCED_MIN)
			#define ID_HOLE (14 == -1 ? ID_DPS_CHANNEL_HOLE : (14 > 0 ? ID_ENHANCED : ID_DEFAULT_HOLE))
			#define ID_RING (14 == -1 ? ID_DPS_CHANNEL_RING : (14 > 0 ? ID_ENHANCED : ID_DEFAULT_RING))
			#define ID_NORM (14 == -1 ? ID_DPS_CHANNEL_NORM : (14 > 0 ? ID_ENHANCED : ID_DEFAULT_NORM))
			...
			#define BuldgeStrength(type) 1.237 * (type == ORIFICE_TYPE_HOLE ? float4(1,1,1,1)[0] : float4(1,1,1,1)[1])
			#define SqueezeStrength(type) 0.442 * (type == ORIFICE_TYPE_HOLE ? float4(1,1,1,1)[0] : float4(1,1,1,1)[1])
			float FindTPSSystem(inout float3 orificePosition, inout float3 orificeNormal, inout float penetratorType, float penetratorLength) {
				...
				#ifdef TPS_IsSkinnedMesh
				return (closestOrfDist) < ((1.0+1) * penetratorLength);
				#else
				return (closestOrfDist) < (min(1.0+1,1.5) * penetratorLength);
				#endif
			}
			...
			void ApplyTPSPenetrator(inout float4 vertex, inout float3 normal, inout float4 vertexColor, uint vertexId, float2 uv) {
				...
				float penetratorLength = 0.3290389;
				float2 shrinkage = float2(1.0, 1.0);
				...
				float bezierSmoothness = 0.17 * penetratorLength;
				float penetrationDepth = 0;
				 if (FindTPSSystem(orificePosition, orificeNormal, orificeType, penetratorLength) )
				{
					#ifdef TPS_IsSkinnedMesh
					float smoothStartStart = 1.0;
					#else
					float smoothStartStart = min(1.0,0.5);
					#endif
					...
					shrinkage = lerp(float2(1.0, 1.0), float2(1, 1), saturate(tpsSmoothStart * 2));
				}
				...
				float3 bezier0 = float4(0,0,1,0) * 0.0;
				...
				float bezierStrengthUncapped = ((penZ - 0.0) / distance(bezier0, bezier1));
				...
				if (orificeType == ORIFICE_TYPE_RING && bezierStrength == 1 && tpsSmoothStart == 1) {
					bezierPoint = orificePosition + (penZ - 0.0 - length(orificePosition.xyz)) * -orificeNormal;
				...
				if (bezierStrength > 0) {
					float sizeChange = 1;
					if (0.054 > 0) {
						sizeChange *= lerp(1 - 0.054 * tpsSmoothStart2, 1 + 0.054 * tpsSmoothStart2, abs(sin(_Time.y * -3.92 + bezierStrengthUncapped / 0.201)));
					}
					float buldgeLerp = 0;
					if (bezierStrength < 1 - 0.02)
					buldgeLerp = saturate((bezierStrength - 1 + 0.402 + 0.02) / 0.402);
					else
					buldgeLerp = saturate((-bezierStrength + 1) / 0.02);
					float squeeze = lerp(0, SqueezeStrength(orificeType), saturate(penetrationDepth * 20));
					sizeChange *= lerp(1, 1 - squeeze, saturate(1 - abs(bezierStrengthUncapped - 1) / 0.158) * tpsSmoothStart);
					sizeChange *= lerp(1, 1 + BuldgeStrength(orificeType), buldgeLerp * tpsSmoothStart2);
					shrinkage = lerp(1, shrinkage, saturate((penZ - 0.0) * 20));
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, indirectLight).xyz, 0.0);
				#endif
				if (1.0)
				{
					#if defined(VERTEXLIGHT_ON)
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.213173,0,0.6396053,1).rgb, 0.0);
