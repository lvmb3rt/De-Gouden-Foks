Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Pro/698721844f79539499094cbbfb38bff1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (_UdonLightVolumeEnabled)
				{
					...
					poiLight.finalLighting += clamp(LightVolumeEvaluate(poiMesh.normals[1], L0, L1r, L1g, L1b), 0.0, 2.5);
					#else
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1890294,0.3467151,0.6231937,1).rgb, 0.0);
