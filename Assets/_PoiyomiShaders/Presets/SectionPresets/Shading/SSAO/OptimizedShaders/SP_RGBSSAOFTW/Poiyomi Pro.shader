Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Pro/373cd0213376b804cb8cd79eadd924d0"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				int totalSamples = int((3.0 * 5) * smoothstep(8.0, 6.0, poiCam.clipPos.w));
				...
				float totalWeight = 0;
				float randomValue = 0.333 * (SSAOInterleavedGradientNoise(uv * _ScreenParams.xy) * 2.0 - 1.0);
				[loop]
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					sampleDir = normalize(lerp(sampleDir, normal, 0.2));
					...
					float depthDifference = depth - sampleDepth - 0.0001;
					float weight = 1;
			...
			float calculateSSAO(PoiMesh poiMesh, PoiCam poiCam, inout PoiLight poiLight, inout PoiMods poiMods)
			{
				...
				float ssao = CalculateAmbientOcclusion(screenPos, depth, transformedNormal, 0.00289, direction, poiMesh, poiCam);
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				float3 ssaoColor = poiThemeColor(poiMods, float4(1,1,1,1).rgb, 0.0).rgb * lerp(1, poiFragData.baseColor, 0.0);
				#ifdef SSAO_COLOR_TEX
				#if defined(PROP_SSAOCOLORMAP) || !defined(OPTIMIZER_ENABLED)
				float2 uv = 1.0 ? ssao : poiMesh.uv[0.0] ;
				ssaoColor *= POI2D_SAMPLER_PAN(_SSAOColorMap, _MainTex, poiUV(uv, float4(1,1,0,0)), float4(3,0,0,0));
				#endif
				#endif
				if (any(float4(0,0,0,2)))
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,2) + 1), (1 - ssao) * 3.0);
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, ssaoColor, 2.0), (1 - ssao) * 3.0);
			}
			#endif
