Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Pro/a9b85a227a5eca54eb8736f6221ed4b3"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (1.0 == 1)
				{
					...
					float passthrough = 0;
					#else
					float passthrough = 0.01;
					#endif
					float2 ToonAddGradient = float2(0.35, 0.4);
				...
				if (1.0)
				{
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 0.1 * blurMap.r, 0.1);
				}
				else
				{
					...
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 0.1 * blurMap.r, 0.1);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				#ifndef POI_PASS_ADD
				indirectColor = lerp(indirectColor, poiLight.directColor, poiLight.indirectColor * 0.666);
				#endif
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (lightingMode == 1)
						{
							float2 ToonAddGradient = float2(0.35, 0.4);
							if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
							vertexLighting = max(vertexLighting, lerp(poiLight.vColor[index], poiLight.vColor[index] * 0.01, smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow);
						}
					}
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,0) + 1), (1 - ssao) * 1.0);
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, ssaoColor, 2.0), (1 - ssao) * 1.0);
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (0.0 == 0) // Poi Custom Light Color
				{
					...
					poiLight.indirectColor = BetterSH9(float4(lerp(0, poiMesh.normals[1], 0.5), 1));
				}
				...
				if (1.0)
				{
					poiLight.directColor = min(poiLight.directColor, 2.0);
					poiLight.indirectColor = min(poiLight.indirectColor, 2.0);
				}
				if (0.0)
				{
				...
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 0.01);
				...
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1811642,0.1811642,0.1811642,1).rgb, 0.0);
