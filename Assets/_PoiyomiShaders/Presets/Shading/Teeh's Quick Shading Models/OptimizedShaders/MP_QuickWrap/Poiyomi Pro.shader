Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Pro/c3bc736fe193e3f43a32e9f2ab399887"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float3 x0, x1, x2;
				float3 conv = lerp(GreenWrapSH(wrap), GreenWrapSHOpt(wrap), 1.0); // Should try optimizing this...
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (1.0 == 1)
				{
					...
					#endif
					float2 ToonAddGradient = float2(0.2, 0.4);
				...
				float3 directColor = _LightColor0.rgb * saturate(RTWrapFunc(poiLight.nDotL, 1.5, 1.0));
				float3 indirectColor = 0;
				#ifdef UNITY_PASS_FORWARDBASE
				indirectColor = ShadeSH9_wrapped(lerp(poiMesh.normals[0], poiMesh.normals[1], 1.0), 1.5) * poiLight.occlusion;
				#endif
				...
				poiLight.rampedLightMap = saturate((poiLight.lightMap - (1 - 1.0)) / saturate((1 - 0.5) - (1 - 1.0) + fwidth(poiLight.lightMap)));
				...
				if (1.0)
				{
					finalWrap = clamp(finalWrap, 0.0, 2.5);
				}
				else
				{
			...
			float CalculateAmbientOcclusion(float2 uv, float depth, float3 normal, float radius, float4 worldDirection, PoiMesh poiMesh, PoiCam poiCam)
			{
				...
				int totalSamples = int((5.16 * 5) * smoothstep(8.0, 6.0, poiCam.clipPos.w));
				...
				float totalWeight = 0;
				float randomValue = 0.1 * (SSAOInterleavedGradientNoise(uv * _ScreenParams.xy) * 2.0 - 1.0);
				[loop]
				for (int sampleIndex = 0; sampleIndex < totalSamples; sampleIndex++)
				{
					...
					sampleDir = normalize(lerp(sampleDir, normal, 0.4));
					...
					float depthDifference = depth - sampleDepth - 0.0;
					float weight = 1;
			...
			{
				if (!DepthTextureExists() || 1.0 == 0) return 1;
				...
				float3 transformedNormal = mul((float3x3)UNITY_MATRIX_V, lerp(poiMesh.normals[0], poiMesh.normals[1], 1.0));
				float mask = 1;
				#ifdef SSAO_MASK_TEX
				...
				float ssao = CalculateAmbientOcclusion(screenPos, depth, transformedNormal, 0.0065, direction, poiMesh, poiCam);
			...
			{
				if (!DepthTextureExists() || 1.0 == 0) return;
				ssao = lerp(ssao, 1, poiLight.rampedLightMap * 0.0);
				float3 ssaoColor = poiThemeColor(poiMods, float4(0.4338796,0.4338796,0.4338796,1).rgb, 0.0).rgb * lerp(1, poiFragData.baseColor, 0.0);
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,0) + 1), (1 - ssao) * 2.5);
				}
				poiFragData.baseColor = lerp(poiFragData.baseColor, customBlend(poiFragData.baseColor, ssaoColor, 2.0), (1 - ssao) * 2.5);
			}
			#endif
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = 1.0 ? MaxLuminance(unity_LightColor[index].rgb * poiLight.vAttenuation[index], 5.0) : unity_LightColor[index].rgb * poiLight.vAttenuation[index];
				...
				poiLight.attenuationStrength = 0.2;
				poiLight.directColor = 1.0 ? MaxLuminance(_LightColor0.rgb * poiLight.attenuation, 5.0) : _LightColor0.rgb * poiLight.attenuation;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				poiLight.indirectColor = 0;
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 0.1);
				poiLight.indirectColor = 1.0 ? MaxLuminance(poiLight.indirectColor, 5.0) : poiLight.indirectColor;
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), 0.0);
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), 0.0);
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
