Shader "Hidden/Locked/.poiyomi/Old Versions/9.2/Poiyomi Pro/3376ba1da5060904fad953f4af5802ee"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.2, 0.0 * blurMap.r);
					lns.y = poiEdgeNonLinearNoSaturate(lightMap.y, 0.0, 0.0 * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(lightMap.z, 0.0, 0.0 * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.2, 0.0 * blurMap.r, 1.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.2, 0.0 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.0, 0.0 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.0, 0.0 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.2, 0.0 * blurMap.r, 1.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				#ifndef POI_PASS_ADD
				indirectColor = lerp(indirectColor, poiLight.directColor, poiLight.indirectColor * 0.333);
				#endif
			...
			void applySSAO(float ssao, in PoiMesh poiMesh, inout PoiFragData poiFragData, inout PoiMods poiMods, inout PoiLight poiLight)
			{
				...
				float3 ssaoColor = poiThemeColor(poiMods, float4(0.6038275,0.6038275,0.6038275,1).rgb, 0.0).rgb * lerp(1, poiFragData.baseColor, 0.0);
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, lilToneCorrection(poiFragData.baseColor, float4(0,0,0,1) + 1), (1 - ssao) * 2.5);
				}
